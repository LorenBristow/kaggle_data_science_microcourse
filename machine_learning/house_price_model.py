...
X = iowa_dataframe.drop(['SalePrice'], axis=1)
X = X.select_dtypes(exclude=['object'])
cols_missing_values = [col for col in X if X[col].isnull().any()]
print('Columns with missing values list: \n', cols_missing_values)
...
#Handle missing values - imputation with extension    
imputer = Imputer()
train_X_imputed_ext = train_X.copy()
validation_X_imputed_ext = validation_X.copy()
for col in cols_missing_values:
    train_X_imputed_ext[col + '_was_missing'] = train_X_imputed_ext[col].isnull()
    validation_X_imputed_ext[col + '_was_missing'] = validation_X_imputed_ext[col].isnull()
train_X_imputed_ext = imputer.fit_transform(train_X_imputed_ext)
validation_X_imputed_ext = imputer.transform(validation_X_imputed_ext)
print('Approach 3 in main model')
